...
const ClubManagementHierarchyScreen = ({ club }) => {
  ...
  function handleAddBoardMember(e) {
    let headers = {
      "Content-Type": "application/json",
      Authorization: "Bearer " + Env.TOKEN,
    };
  ...
  function handleDeleteBoardMember(boardMember){
    let headers = {
      ...
      axios.delete(process.env.REACT_APP_URL + "club/boardMember?boardMemberId=" + boardMember.id, {headers: headers})
      .then(function (response) {
        if (response.status === 200) {
          setSearchText("")
                    ...
                    >
                      ...
                      >
                        <Grid container style={{ alignItems: "stretch" }}>
                          ...
                          >
                            <IconButton
                              onClick={() => {handleDeleteBoardMember(boardMember)}}
                              style={{
                                borderRadius: 0,
                              }}
                            >
