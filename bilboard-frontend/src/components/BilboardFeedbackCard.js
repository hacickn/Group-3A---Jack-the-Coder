...
const BilboardFeedbackCard = ({ feedback }) => {
  ...
  const [statusOfFeedback, setStatusOfFeedback] = React.useState(feedback.status);
    ...
    axios
      .post(
        process.env.REACT_APP_URL +
          "club/feedback/respond?feedbackId=" +
          feedback.id +
          "&status=" +
          status,
        {},
  ...
  return (
    ...
    >
      ...
      <Grid item xs={2}>
        Current status: 
        {statusOfFeedback ? <div>Accepted</div> : <div>Declined</div>}
      </Grid>
      <Grid item xs={3}>
        <Grid
          container
